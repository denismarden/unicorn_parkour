...
function renderLeaderboard() {
    ...
    if (leaderboard.length === 0) {
        list.innerHTML = '<p style="text-align:center; color:#aaa;">Nenhum recorde ainda!</p>';
        return;
    }
    ...
    leaderboard.forEach((entry, index) => {
        ...
        let cName = entry.charName ? `<span class="lb-char" style="color: #00f0ff; font-size: 0.9em; margin-left: 5px;">(${entry.charName} - Fase ${entry.phase || 1})</span>` : '';
        div.innerHTML = `<span class="lb-rank">#${index + 1}</span>
...
function checkHighScore(finalPt) {
    if (leaderboard.length < 10 || finalPt > leaderboard[leaderboard.length - 1].score) {
        nameInputContainer.classList.remove('hidden');
        btnRestart.classList.add('hidden'); // Hide restart until saved
...
btnSaveScore.onclick = () => {
    ...
    leaderboard.push({ name: name, score: Math.floor(score), charName: player.char.name, phase: currentPhase + 1 });
